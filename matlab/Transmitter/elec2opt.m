...
function [ovosignal, phase_noise] =  elec2opt( esignal, laser, MZmod, txedfa, sim, params )
    [d1, d2] = size(esignal);
...
% 4. up-conversion for NLSE simulation 
    ...
    if (sim.nonoise == 1 || sim.en_AWGN == 0 )
        ...
        noise=AddAWGN(optoofdmout_pz, sim.SNR , sim);
        noisyoptoofdmout_pz = optoofdmout_pz+noise;
    end
%     noisyoptoofdmout_pz=optoofdmout_pz;
    %% Upsampling 
    if ( sim.oversample == 1 )
