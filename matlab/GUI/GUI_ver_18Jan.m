function varargout = GUI_ver_18Jan(varargin)
% GUI_VER_18JAN M-file for GUI_ver_18Jan.fig
%      GUI_VER_18JAN, by itself, creates a new GUI_VER_18JAN or raises the existing
%      singleton*.
%
%      H = GUI_VER_18JAN returns the handle to a new GUI_VER_18JAN or the handle to
%      the existing singleton*.
%
%      GUI_VER_18JAN('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_VER_18JAN.M with the given input arguments.
%
%      GUI_VER_18JAN('Property','Value',...) creates a new GUI_VER_18JAN or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_ver_18Jan_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_ver_18Jan_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_ver_18Jan

% Last Modified by GUIDE v2.5 20-Jan-2014 15:22:16

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_ver_18Jan_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_ver_18Jan_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_ver_18Jan is made visible.
function GUI_ver_18Jan_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = GUI_ver_18Jan_OutputFcn(hObject, eventdata, handles) 
...
[FileName,PathName]  = uigetfile('D:\user\haeyoung\Project\2014\OSC_Data\13Jan14\*.*');
...
function [sim, params, couti, coutq, offset, maxidx, fiber]= InitStart(hObject, eventdata, handles)
    ...
    if ( get(handles.checkbox21,'Value') == 1 )
            [sim, params, MZmod,fiber,laser, iolaser, txedfa, edfa, rxedfa, PD]= ...       
                 InitOFDM_22Jan( NFFT, RxOVERSAMPLE, handles.SampleTime *  stepidx  );
...
%                  InitOFDM_06Sept( NFFT, RxOVERSAMPLE, handles.SampleTime *  stepidx  );   
    ...
    sim.stepidx = stepidx;
    sim.en_OFDE=0;
    sim.fixed_sim = get(handles.checkbox31,'Value');  
    sim.cetype = get(handles.checkbox32,'Value') + 6; 
...
%     
    if ( sim.fixed_sim == 1 )
            cout = NRxAGC (complex(couti, coutq), params, sim);
            cout = Change_fixed_bit_lim(cout, sim.ADCbit, 2^(sim.ADCbit-1)-1); 
            couti = real(cout);
            coutq = imag(cout);
    end 
...
if (  get(handles.checkbox6,'Value') == 1 )
    time_signal =complex(handles.eouti, handles.eoutq);%handles.maxoffset.noisyreceivein(1,:);
    pol_str=' X pol ';
else
    time_signal =complex(handles.outi, handles.outq);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[file,path] = uiputfile('D:\User\Haeyoung\COOFDM\OSC_Data\22Jan14\*.dat','Save Frame data  As'); 
dlmwrite( [path, 'C1_', file], (handles.eouti )','newline' , 'pc');
dlmwrite( [path, 'C2_', file], (handles.outi)','newline' , 'pc');
dlmwrite( [path, 'C3_', file], (handles.eoutq)','newline' , 'pc');
dlmwrite( [path, 'C4_', file], (handles.outq)','newline' , 'pc');
disp(['File write', path, ' ', file]);
